...
public class MeleeWeaponHandler : WeaponHandler
{
    ...
    {
        if (Controller == null) return; // Controller 없으면 스킵 (에디터 경고 방지)

        Vector3 boxCenter = transform.position + (Vector3)Controller.LookDirection * collideBoxSize.x;
        Gizmos.color = Color.red;
        Gizmos.DrawWireCube(boxCenter, collideBoxSize);
    }
